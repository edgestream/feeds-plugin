...
export function createFeedsMcpServer({ feeds, version = "0.1.0", timeoutMs = 60_000, resultBytes = 10 * 1024 * 1024 }: FeedsMcpOptions): McpServer {
  ...
  async function retrieve(source: string, options: { context?: boolean | undefined; answers?: boolean | undefined; cursor?: string | undefined; limit?: number | undefined; all?: boolean | undefined }, caller: AbortSignal) {
    ...
    try {
      ...
      if (!parsed.success) throw new FeedError("INVALID_RESPONSE", "Invalid feed result structure.", { cause: parsed.error });
      return result;
    } catch (error) {
      if (caller.aborted) throw new FeedError("CANCELLED", "Request cancelled.", { cause: error, diagnostics: { abortReason: caller.reason } });
      if (deadline.aborted) throw new FeedError("TIMEOUT", `MCP request exceeded its total time budget. Retry one page without all (or with all: false), then pass each returned nextCursor as cursor with the same source and scope. Example: {"source":"OpenAI","limit":25}.`, { cause: error, diagnostics: { abortReason: deadline.reason } });
      throw error;
    }
  }
  ...
  server.registerResource("feed", new ResourceTemplate("feeds://{platform}/{reference}", { list: undefined }), {
    ...
    try {
      ...
      const failure = failureData(safeFailure(error), resultBytes);
      throw new ProtocolError((failure.code === "INVALID_INPUT" || failure.code === "NOT_FOUND") ? ProtocolErrorCode.InvalidParams : ProtocolErrorCode.InternalError, failure.code, failure);
    }
  });
  return server;
}
