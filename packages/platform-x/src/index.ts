import { FeedError, type Platform, type PostRef } from "@edgestream/feeds-core";
...
export class XPlatform implements Platform {
  ...
  resolve(url: URL): PostRef | undefined {
    ...
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || url.port || !match) {
      throw new FeedError("INVALID_INPUT", "Expected a public X post URL: https://x.com/handle/status/id");
    }
