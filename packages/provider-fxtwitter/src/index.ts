...
export class FxTwitterProvider implements FeedProvider {
  ...
  async get(query: FeedQuery, context: RequestContext = {}): Promise<FeedPage> {
    ...
    const payload = await this.request(`${path}${params.size ? `?${params}` : ""}`, context, subject.kind === "author");
  ...
  private async request(path: string, context: RequestContext, allowEmpty: boolean): Promise<JsonObject> {
    ...
    try {
      response = await this.fetch(`https://api.fxtwitter.com/2/${path}`, {
