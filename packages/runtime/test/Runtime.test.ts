...
test("shares provider defaults and explicit environment configuration", async () => {
  assert.deepEqual(readConfiguration({}), { xProvider: "fxtwitter", blueskyProvider: "fxtwitter" });
  assert.deepEqual(readConfiguration({ FEEDS_X_PROVIDER: "other" }), { xProvider: "other", blueskyProvider: "fxtwitter" });
...
test("composes independent X and Bluesky providers without cross-routing", async () => {
  assert.deepEqual(readConfiguration({ FEEDS_BLUESKY_PROVIDER: "custom" }), { xProvider: "fxtwitter", blueskyProvider: "custom" });
  for (const blueskyProvider of ["", "other", "constructor"]) assert.throws(() => createFeed({ xProvider: "fxtwitter", blueskyProvider }), { code: "CONFIGURATION" });
  const calls: string[] = [];
  const service = createFeed({ xProvider: "fxtwitter" }, { fetch: async url => { calls.push(String(url)); return Response.json({ raw: url }); } });
...
test("rejects AT URIs and bare Bluesky identifiers at the application boundary", async () => {
  const service = createFeed({ xProvider: "fxtwitter" }, { fetch: async () => assert.fail("must not fetch") });
