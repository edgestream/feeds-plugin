...
export function feedProviderContract(create: () => FeedProvider, expected: JsonObject): void {
  ...
  test("provider contract: supports cancellation", async () => {
    await assert.rejects(create().get(new URL("https://x.com/alice/status/123"), {}, { signal: AbortSignal.abort() }),
